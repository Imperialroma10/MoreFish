...
public class RewardItem extends RewardAbstract {
    ...
    public String getRewardMessage() {
        ...
        return main.getPlugin().getChecker().getString("plugin-prefix").toString() + main.getPlugin().getChecker().getString("item-reward-message").toString().replace("{itemname}", ItemUtil.getMaterialName(item.getType().name())).replace("{count}", item.getAmount() + "");
    }
